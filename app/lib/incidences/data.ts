...
export async function fetchOrderLinesByIncidence(
  ...
  const sqlText = `
    SELECT
      sol.ORDER_ID,
      sol.SUBORDER_ID,
      sol.ECOMMERCE_NAME_CHILD,
      sol.ECOMMERCE_NAME,
      sol.DELIVERY_METHOD_NAME,
      sol.DATE_ORDER,
      sol.DEFAULT_CODE,
      sol.PRODUCT_NAME,
      sol.PRODUCT_UOM_QTY,
      sol.WAREHOUSE,
      sol.TRANSFER_WAREHOUSE,
      sol.STATE_OPTION_NAME,
      so.ORDER_ID AS SHOPIFY_ORDER_ID
    FROM PATAGONIA.CORE_TEST.OMS_SUBORDERSLINE sol
    LEFT JOIN PATAGONIA.CORE_TEST.SHOPIFY_ORDERS so
      ON sol.ECOMMERCE_NAME = so.NAME
    WHERE sol.ECOMMERCE_NAME = (
